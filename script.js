const correctPassword = "Nattiva2026";
...
cuota.addEventListener("input", () => {
  cuota.value = cleanNumericInput(cuota.value);
});

tea.addEventListener("input", () => {
  tea.value = cleanNumericInput(tea.value);
});

plazo.addEventListener("input", () => {
  plazo.value = plazo.value.replace(/[^\d]/g, "");
});

cliente.addEventListener("input", () => {
  cliente.value = cliente.value.replace(/[0-9]/g, "");
});
...
function calcular() {
  ...
  if (c <= 0 || c >= 100) {
    err.textContent = "La cuota inicial debe ser mayor a 0 y menor a 100.";
    return;
  }

  if (t <= 0) {
    err.textContent = "La TCEA debe ser mayor a 0.";
    return;
  }

  if (a <= 0) {
    err.textContent = "El plazo debe ser mayor a 0 años.";
    return;
  }
  ...
  const cuotaMensualCalc =
    montoFinanciar * (tasaMensual / (1 - Math.pow(1 + tasaMensual, -meses)));
...
function compartirEmail() {
  const subject = encodeURIComponent("Resumen de crédito Nattiva");
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        title: "Resumen de crédito Nattiva",
        text: texto
      });
    } catch (e) {
...
function descargarPDF() {
  ...
  if (!element) {
    alert("No se encontró el contenido para exportar en PDF.");
    return;
  }
  ...
  const options = {
    margin: 0.4,
    filename: `Resumen-Credito-${clientName}.pdf`,
